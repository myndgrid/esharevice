...
*Last updated: 2026-05-16 18:18 UTC | Global SWE Agent Config | Adapt the Architecture and Project Structure sections per project — everything else applies universally. Bug registry: 48 entries (+1 from wiring Lighthouse CI auth for the lh-bot user — Authentik's flow-executor enforces CSRF via `X-Authentik-CSRF`, not Django's stock `X-CSRFToken`, and the resulting error payload is opaque without the server-side request_id grep).*