...
- **Object storage:** Cloudflare R2 for images, keyed by `sha256(content)/<width>.webp` for three width variants (1600/800/400). Wired live 2026-05-16 via dashboard-only S3-compatible API token + a `cdn.esharevice.com` custom-domain binding (Cloudflare's public REST API doesn't expose either of those operations — captured in the bug registry). The `/v1/exchange-items/:id/image` endpoint env-gates on `r2Configured()` so dev/test still runs without the keys.
- **Env files are split per-process, never consolidated.** Locally and in prod, the API and the web app read separate env stores. The API uses `apps/api/.env` (loaded via tsx `--env-file=.env`) and gets `DATABASE_URL`, `REDIS_URL`, `OIDC_ISSUER`/`AUDIENCE`/`JWKS_URL`, `R2_*`, `CDN_BASE_URL`, `WEB_ORIGIN`. The web uses `apps/web/.env.local` (Next.js auto-loads) and gets `NEXT_PUBLIC_API_URL`, `OIDC_CLIENT_ID`, `OIDC_CLIENT_SECRET`, `OIDC_REDIRECT_URI`, `SESSION_COOKIE_SECRET`. Only `OIDC_ISSUER` is duplicated. The API never sees the client secret; the web never sees the database URL — that's a security boundary worth keeping. Same shape in `infra/docker-compose.yml` where each service has its own `environment:` block.
...
### [Security] Authentik Uses a Non-Default CSRF Header Name on `/api/v3/flows/executor/*`
**Description:** When driving Authentik's flow-executor JSON API programmatically (e.g. for a Lighthouse-CI puppeteerScript that authenticates a test user), the identification + password stages POST cleanly without any CSRF header — DRF's `SessionAuthentication` only enforces CSRF once the session is authenticated. The consent stage (`ak-stage-consent`) is the FIRST post-login POST, so that's where CSRF kicks in. Sending Django's stock `X-CSRFToken` header (with the value from the `authentik_csrf` cookie) gets you `ak-stage-flow-error` with only a `request_id` — opaque from the client. The server-side exception trace says `rest_framework.exceptions.PermissionDenied: CSRF Failed: CSRF token missing.` because Authentik configures `CSRF_HEADER_NAME = HTTP_X_AUTHENTIK_CSRF`, which maps to the HTTP header `X-Authentik-CSRF`, not `X-CSRFToken`.
**Avoid:** When making programmatic POSTs against Authentik's flow-executor (or any other authenticated `/api/v3/*` endpoint), use `X-Authentik-CSRF: <value from authentik_csrf cookie>` as the header. Confirm via `docker exec <authentik> python -c "from django.conf import settings; print(settings.CSRF_HEADER_NAME)"`. To diagnose any opaque flow-error: `docker logs esharevice-authentik-server-1 | grep <request_id>` — the actual exception trace ships in the server logs only.

---

### [Security] `startsWith("/")` Is Not a Sufficient Open-Redirect Guard — Protocol-Relative URLs Escape
**Description:** An OAuth/OIDC callback that gates its post-login redirect with `target.startsWith("/")` accepts `//evil.com/phishing`, `/\evil.com`, and `///evil.com`. The URL constructor resolves protocol-relative inputs against the base origin: `new URL("//evil.com/x", "https://app.example.com")` returns `https://evil.com/x`. The redirect then lands on the attacker's origin — even though every check "looked like" it was rejecting external URLs. The /cso 2026-05-16 audit caught this exact pattern in our `apps/web/app/api/auth/callback/route.ts` (Finding #1). Confidence-9 phishing chain: the attacker sends a victim to `https://esharevice.com/api/auth/login?return_to=%2F%2Fevil.com%2F`, the victim authenticates on the real Authentik, the callback redirects them to evil.com which serves a fake "session expired" form. The brand domain was real the whole time.
**Avoid:** Never rely on string-prefix checks for redirect validation. Resolve the candidate via the URL constructor against the app's `publicOrigin`, then compare `candidate.origin` against `baseOrigin`. On mismatch OR any throw, fall through to a safe default (`/`). Only use `candidate.pathname + candidate.search + candidate.hash` for the final redirect target — never reflect a full URL back, even a same-origin one. Pattern:
```ts
...
```
Same guard applies to ANY redirect built from a request parameter or stored value — error pages, post-action toasts, magic-link confirm flows.
